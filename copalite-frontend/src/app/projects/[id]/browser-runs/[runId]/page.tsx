...
export default function BrowserRunDetailPage() {
  ...
  async function loadData() {
    ...
    try {
      ...
      if (r.projectId) {
        ...
        setAllRuns(
          runs.filter(
            (br: BrowserRunResponse) =>
              br.status === "completed" && br.id !== runId,
          ),
        );
      }
    } catch (err: unknown) {
  ...
  async function handleExport(format: "markdown" | "pdf") {
    ...
    const url = `${API_URL}/browser-specs/by-run/${runId}/${format}${q}`;
    ...
    try {
      ...
      a.download = `spec_${runId.substring(0, 8)}_${new Date().toISOString().split("T")[0]}.${ext}`;
      a.click();
  ...
  return (
    <div className="flex min-h-screen">
      ...
      <main className="flex-1 ml-[260px]">
        <Header
          title={run?.journeyName || run?.module || "Browser Run"}
          subtitle={
            run
              ? `${run.stepsCount} steps, ${run.evidencesCount} evidencias`
              : ""
          }
        />
        <div className="p-8 space-y-6">
          {error && (
            <div className="card p-8 text-center border-rose-500/20">
              ...
              <button onClick={loadData} className="btn-secondary mt-4 gap-2">
              ...
              <>
                ...
                <div className="card p-5">
                  <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
                    <div>
                      <p className="text-[10px] text-coal-500 uppercase">
                        Status
                      </p>
                      <span
                        className={`text-xs ${RUN_STATUS_BADGE[run.status]}`}
                      >
                        {run.status}
                      </span>
                    </div>
                    <div>
                      ...
                      <p className="text-xs text-coal-200">
                        {run.target?.name || "—"}
                      </p>
                    </div>
                    <div>
                    ...
                    <div>
                      <p className="text-[10px] text-coal-500 uppercase">Fim</p>
                      <p className="text-xs text-coal-200">
                        {run.finishedAt
                ...
                {summary && summary.total > 0 && (
                  <ProblemSummaryCards summary={summary} />
                )}
                ...
                <div className="flex items-center gap-2">
                  <button
                    onClick={() => setTab("spec")}
                    className={`px-3 py-1.5 rounded-lg text-xs font-medium transition-all ${tab === "spec" ? "bg-emerald-600/20 text-emerald-400 border border-emerald-500/30" : "text-coal-400 hover:text-coal-200 bg-coal-800/40 border border-coal-800"}`}
                  >
                    <FileText size={12} className="inline mr-1" /> Spec
                  </button>
                  <button
                    onClick={() => setTab("problems")}
                    className={`px-3 py-1.5 rounded-lg text-xs font-medium transition-all ${tab === "problems" ? "bg-emerald-600/20 text-emerald-400 border border-emerald-500/30" : "text-coal-400 hover:text-coal-200 bg-coal-800/40 border border-coal-800"}`}
                  >
                    <ShieldAlert size={12} className="inline mr-1" /> Problemas
                    ({problems.length})
                  </button>
                  <button
                    onClick={() => setTab("evidence")}
                    className={`px-3 py-1.5 rounded-lg text-xs font-medium transition-all ${tab === "evidence" ? "bg-emerald-600/20 text-emerald-400 border border-emerald-500/30" : "text-coal-400 hover:text-coal-200 bg-coal-800/40 border border-coal-800"}`}
                  >
                    <Camera size={12} className="inline mr-1" /> Evidencias (
                    {evidences.length})
                  </button>
                </div>
                  ...
                  <div className="space-y-6">
                    ...
                    {allRuns.length > 0 && (
                      <div className="card p-3">
                        <label className="text-xs text-coal-400 mr-2">
                          Comparar com run anterior:
                        </label>
                        <select
                          className="input-field w-64 text-xs inline-block"
                          value={baseRunId}
                          onChange={(e) => handleBaseRunChange(e.target.value)}
                        >
                    ...
                    {specHistory.length > 0 && (
                      <div className="card p-3">
                        <p className="text-[10px] text-coal-500 mb-2">
                    ...
                    {specLoading && (
                      <div className="flex items-center justify-center h-32">
                        <Loader2
                          className="animate-spin text-coal-500"
                          size={24}
                        />
                      </div>
                    )}
                    ...
                    {!specLoading && !specError && spec && (
                      <>
                        ...
                        {spec.diffSummary && (
                          <SpecDiffSummary diff={spec.diffSummary} />
                        )}
                    ...
                    {!specLoading && !specError && !spec && (
                      <div className="card p-8 text-center">
                        <FileText
                          className="mx-auto text-coal-600 mb-3"
                          size={32}
                        />
                        <p className="text-coal-400 text-sm">
                          Spec nao disponivel para esta run
                        </p>
                      </div>
                    )}
                  </div>
                          ...
                          <div key={ev.id} className="card p-4 animate-fade-in">
                            <div className="flex items-start gap-3">
                              <div className="flex items-center gap-2 shrink-0">
                                <span className="text-[10px] text-coal-600 w-6 text-right">
                                  #{ev.stepIndex}
                                </span>
                                <div className="p-1.5 rounded bg-coal-800/50">
                              ...
                              <div className="flex-1 min-w-0">
                                <div className="flex items-center gap-2 mb-1">
                                  ...
                                  {ev.action && (
                                    <span className="text-xs text-coal-200">
                                      {ev.action}
                                    </span>
                                  )}
                                </div>
                                {ev.route && (
                                  <p className="text-[10px] text-coal-500 font-mono truncate">
                                    {ev.route}
                                  </p>
                                )}
                                <span className="text-[9px] text-coal-600">
