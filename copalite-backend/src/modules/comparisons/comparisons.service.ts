...
export class ComparisonsService {
  ...
  async findAllByProject(projectId: string) { return this.compRepo.find({ where: { projectId }, order: { createdAt: 'DESC' }, take: 500 }); }
  ...
  async listDiffs(comparisonId: string) { return this.diffRepo.find({ where: { comparisonId }, order: { createdAt: 'ASC' }, take: 500 }); }
}
