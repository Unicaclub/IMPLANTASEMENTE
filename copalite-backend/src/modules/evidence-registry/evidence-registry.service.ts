...
export class EvidenceRegistryService {
  ...
  async findAllByRun(runId: string) { return this.repo.find({ where: { runId }, order: { createdAt: 'DESC' } }); }
  async findAllByRelatedEntity(relatedEntityType: string, relatedEntityId: string) {
    return this.repo.find({ where: { relatedEntityType, relatedEntityId }, order: { createdAt: 'DESC' } });
  }
